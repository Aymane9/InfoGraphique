...
// Rendering an image from OBJ models
...
public:
    ...
    virtual bool intersection(const Ray& r, Vector3& P, Vector3& Normale, double& t) = 0;
    Vector3 albedo;
...
public:
    ...
    bool intersection(const Ray& r, Vector3& P, Vector3& Normale, double& t) {
        if (!BVH->b.intersection(r)) return false; 
        t = 1E9;
        ...
        while (!l.empty()) {
            ...
            if (c->fg) { 
                ...
                for (int i = c->debut; i < c->fin; i++) {
                    ...
                    if (beta >= 0 && gamma >= 0 && alpha >= 0 && beta <= 1 && gamma <= 1 && alpha <= 1 && tObject > 0) { 
                        intersecte = true;
                        if (tObject < t) { 
                            t = tObject;
...
public:
    ...
    bool intersection(const Ray& r, Vector3& P, Vector3& N, Vector3& albedo, double& t, bool& Mirror, bool& transp, int& id) {
        ...
        for (int i = 0; i < Objects.size(); i++) {
            ...
            double tObject;
            if (Objects[i]->intersection(r, PObject, NObject, tObject) && tObject < t) { 
                intersecte = true;
                t = tObject;
                P = PObject;
                N = NObject;
...
int main() {
    ...
    Sphere Slight(scene.L, 5, Vector3(1., 0.3, 0.2));
    ...
    m.Pivot(m.BVH, 0, m.indices.size());
